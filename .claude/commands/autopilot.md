...
- After commit succeeds → update stage to "finalise", continue to 3e

### 3e — Finalise Tasks
After a successful commit, run /coder one final time with this specific instruction:

  "Do not implement anything new. Read TASKS.md and MEMORY.md only.
   For every task currently in ## In Progress:
   - Verify review log shows no Resolved: no entries for that task
   - Verify test log shows no failures for that task
   - If both are clear, mark the task [x] and move it to ## Done
   After updating TASKS.md, stop immediately."

Then check TASKS.md:
- If ## In Progress is now empty → update stage to "stop", continue to Step 4
- If ## In Progress still has tasks → those tasks have unresolved issues
  - Report remaining tasks to user
  - Pause (see Pause Conditions)
...
- Step 3e finds tasks still in ## In Progress after finalise
